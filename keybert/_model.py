...
class KeyBERT:
    ...
    def extract_keywords(
        ...
        data = []
        with open('indonesian-adjective-sentiment-raw.csv', newline='') as csvfile:
            reader = csv.reader(csvfile, delimiter=',')
            for row in reader:
                data.append(row[1])

        verb = []
        with open('word-verb.csv', newline='') as csvfile:
            reader = csv.reader(csvfile, delimiter=',')
            for row in reader:
                verb.append(row[0])
        ...
        # noun_adjective Keywords
        noun_adj_keywords = []
        for bigram_word, value in sorted(all_keywords, key=lambda x: x[1], reverse=True):
